...
if is_year_end:
    ...
    annual_data.append({
        "Year": year_number,
        # Show END-OF-YEAR AGE so the last row displays '65' when ret_age=65
        "Age": int(age_end),                       # <-- changed from age_start to age_end
        "AgeEnd": int(age_end),                    # keep if you still want the field
        "StartingSavings": float(current_savings), # flat baseline band
...
# ── Data Table ────────────────────────────────────────────────────────────────
# Show Age as END-OF-YEAR age so the table ends at the retirement age (e.g., 65)
view_cols = [
    "Year",
    "Age",              # end-of-year age (will show 65 for ret_age=65)
    "StartingSavings",
